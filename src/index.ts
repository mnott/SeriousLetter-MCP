...
const server = new McpServer(
  ...
  },
  {
    instructions: [
      ...
      "",
      "**IMPORTANT**: Evaluation notes are stored as markdown and rendered with full formatting.",
      "Use rich markdown structure: headings, bold scores, bullet lists with concrete reasoning.",
      "Every criterion gets its own `### Heading` with a bold score and bullet points explaining the match.",
      "",
      "Output format for evaluation notes (use this exact structure):",
      "```",
      "## Evaluation: [Position] at [Company]",
      "",
      "**Overall Fit: X/5 - [One-word verdict: Excellent Match / Strong Match / Good Match / Partial Match / Poor Match]**",
      "",
      "### [Criterion Name from preferences]",
      "",
      "**[Criterion Name] (X/5)**",
      "- [Specific evidence from JD matched to CV — be concrete, name technologies/roles/achievements]",
      "- [Another concrete match point]",
      "- [If gap exists, state it honestly]",
      "",
      "### [Next Criterion]",
      "",
      "**[Next Criterion] (X/5)**",
      ...
      "",
      "[Repeat for ALL criteria from user preferences]",
      "",
      "### Location & Logistics",
      "",
      "- **Location**: [City, region — commute/relocation assessment]",
      "- **Work arrangement**: [Remote/Hybrid/On-site — match to preferences]",
      "- **Type**: [Contract type, duration]",
      "",
      "### Pros",
      "",
      "- [Key advantage 1]",
      ...
      "### Summary",
      "",
      "[2-3 sentence overall assessment with clear recommendation: pursue, consider, or skip]",
      "```",
      "",
      "**Quality rules for evaluations:**",
      "- Be **specific**: reference actual JD requirements and actual CV experience by name",
      "- Every bullet must cite evidence (not generic statements like 'strong match')",
      "- Score honestly: 3/5 is fine if the match is partial — don't inflate",
      "- Always include Pros/Cons sections — helps the user make quick decisions",
      "- Use `sl_add_note` with category `evaluation` to save the note",
      "",
      "### Job Status Mapping",
      "",
      "| Status | When to use |",
      ...
      "---",
      "",
      "## Jobs Workflow (Job Application Management)",
      "",
      "USE WHEN: user pastes a job description, mentions job application, cover letter, JD evaluation,",
      "job fit, apply for, bewerbung, motivationsschreiben, update job status, OR says /jobs.",
      "",
      "### Pre-Action Check (MANDATORY)",
      "",
      "Before any cover letter, evaluation, or job creation task:",
      "1. Call `sl_get_preferences()` — always fetch fresh, do not cache",
      "2. Apply `evaluation_criteria` for scoring; `cover_letter_notes` for style",
      "3. Check `availability` and `availability_date` for closing paragraph",
      "4. Current-message instructions override preferences (session override, not persisted)",
      "",
      "### Workflows",
      "",
      "| Workflow | Trigger |",
      "|----------|---------|",
      "| **NewJob** | User pastes JD, 'new job', '/jobs new', 'bewerbung' |",
      "| **BatchAnalyze** | 'here's a mail with jobs', 'analyze these jobs', LinkedIn email |",
      "| **CoverLetter** | 'write cover letter', 'cover letter', 'motivationsschreiben' |",
      "| **Evaluate** | 'evaluate this JD', 'fit assessment', 'how do I fit' |",
      "| **Continue** | '/jobs continue', resume discussion about a job |",
      "| **UpdateStatus** | 'update job', 'job status', 'update <company>', '<company>: <status>' |",
      "| **ORP Entry** | 'add ORP', 'job-room', 'arbeitsbemühung', 'orp entry' |",
      "",
      "### Action Defaults",
      "",
      "- **Cover letter**: Always produce draft for review — never export or submit without explicit confirmation",
      "- **Job creation**: Always check `sl_list_companies` for existing company before creating a new one",
      "- **CV copy**: Always run `sl_copy_cv_to_job` after job creation (inherits exec summary + portfolio)",
      "",
      "### Anti-Defaults (never do these)",
      "",
      "- Never send application emails without user review (use gmail draft via Workspace skill)",
      "- Never overwrite an existing cover letter — use `sl_create_letter` (new version) not `sl_update_letter`",
      "  Exception: use `sl_update_letter` only for explicit in-place edits (typo fixes, minor wording tweaks)",
      "",
      "### Conventions",
      "",
      "- Folder naming: `NNNN - YYYY-MM-DD - Title/` (4-digit zero-padded, globally sequential)",
      "- Main file: same name as folder + `.md`; attachments: `attachments/attachment_N.md`",
      "- Artifacts: `artifacts/Descriptive_Name.md` (cover letters, prep notes)",
      "- Every `.md` file ends with `#jobs`",
      "- NEVER use em-dashes (—) or en-dashes (–) — always use hyphens (-) instead",
      "",
      "---",
      "",
      "## JobExplore Workflow (Browser-Based Job Board Exploration)",
      "",
      "USE WHEN: user mentions explore jobs, job board, scrape jobs, learn site, run recipe,",
      "job exploration, browse jobs, new recipe, add job site, or wants to search job boards automatically.",
      "",
      "Two-phase system using Playwright MCP:",
      "- **Learn mode** (headed browser, interactive) — builds site recipes saved to `~/.claude/Skills/JobExplore/recipes/`",
      "- **Run mode** (headless) — executes recipes, extracts listings, integrates with Jobs workflows",
      "",
      "### Workflows",
      "",
      "| Workflow | Trigger |",
      "|----------|---------|",
      "| **Learn** | 'learn site', 'new recipe', 'add job board', 'teach you <site>' |",
      "| **Run** | 'run recipe', 'explore jobs', 'search <site>', 'check job boards' |",
      "",
      "### Recipe Format",
      "",
      "Each recipe is a markdown file in `~/.claude/Skills/JobExplore/recipes/` with sections:",
      "Meta (URL, auth, anti-bot, status), Auth (if needed), Search (URL template, wait-for selector),",
      "Job Cards (container, card selector, field selectors), Pagination, Detail Page (optional).",
      "",
      "### Integration",
      "",
      "When a listing is found during Run:",
      "1. Extract title, company, description, URL",
      "2. Offer to evaluate via Jobs Evaluate workflow",
      "3. If user wants to apply, trigger Jobs NewJob workflow with extracted JD",
      "4. Optionally create job in SeriousLetter via `sl_create_job`",
    ].join("\n"),
  },
);
